...
- **Item 3.2 — escopo:** a lista de espera só existe em evento com aprovação manual (não há fila de "esgotado" em evento aberto); a promoção não reserva estoque (só libera o direito de comprar), então alguém pode achar o lote esgotado se outro não-aprovado... não: só aprovados compram, mas aprovados > vagas nunca acontece por construção. Cancelar evento não dispara promoção.
